...
#include <stddef.h>
#include "console.h"
#ifdef PLATFORM_SG2002
#include "../../../arch/rv64/sg2002/kernel/drivers/sdhci.h"
#define block_read(lba, buffer) sdhci_read_block((unsigned long)(lba), (buffer))
#define block_write(lba, buffer) sdhci_write_block((unsigned long)(lba), (buffer))
#else
#include "ata.h"
#define block_read(lba, buffer) block_read((lba), (uint16_t *)(buffer))
#define block_write(lba, buffer) block_write((lba), (uint16_t *)(buffer))
#endif
#include "fat32.h"
...
static int fat32_read_sector(uint32_t lba, uint8_t *buffer) {
    return block_read(lba, (uint16_t *)buffer);
}
...
static int fat32_write_fat_entry(uint32_t cluster, uint32_t value) {
    ...
    *(uint32_t *)&sector[index] = entry;
    ...
    return block_write(lba, (uint16_t *)sector);
}
...
int fat32_write(const char *path, uint64_t offset, const void *buffer, size_t size) {
    ...
    while (bytes_written < size && !fat32_is_end_of_chain(current_cluster)) {
        for (uint32_t sector_idx = 0;
            ...
            block_write(lba, (uint16_t *)sector);
        }
