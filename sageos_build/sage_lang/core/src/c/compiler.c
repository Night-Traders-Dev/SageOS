...
static const char *compiler_unknown_name_help(Compiler *compiler,
                                              const char *name,
                                              const char *fallback,
                                              char *buffer,
                                              size_t buffer_size) {
...
static void compiler_builtin_arity_error(Compiler *compiler, CallExpr *call,
  ...
  compiler_error_at(compiler, expr_token(call->callee), usage,
                    ...
                    strcmp(expected, "1") == 0 ? "" : "s", call->arg_count);
}
...
static char *resolve_module_path_for_compiler(const Compiler *compiler,
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (dlen + slen + plen + 6 >= sizeof(path))
      continue;
    strcpy(path, dir);
  ...
  while (stmt != NULL) {
    ...
    case STMT_TRY: {
      ...
      for (int i = 0; i < stmt->as.try_stmt.catch_count; i++) {
        ...
        free(catch_var);
        collect_local_lets(compiler, stmt->as.try_stmt.catches[i]->body,
                           locals);
      }
  ...
  while (stmt != NULL) {
    ...
    case STMT_LET: {
      ...
      if (find_proc_entry(compiler->procs, name) != NULL) {
        compiler_error(compiler, "global '%s' conflicts with procedure name",
                       name);
      } else {
    ...
    case STMT_FOR: {
      ...
      if (find_proc_entry(compiler->procs, var_name) != NULL) {
        compiler_error(compiler,
                       "for-loop variable '%s' conflicts with procedure name",
                       var_name);
      } else {
...
static void process_import(Compiler *compiler, ImportStmt *import) {
  ...
  char *module_path =
      resolve_module_path_for_compiler(compiler, import->module_name);
  if (module_path == NULL) {
  ...
  if (source == NULL) {
    compiler_error(compiler, "cannot read module '%s' at '%s'",
...
static char *emit_slice_expr(Compiler *compiler, SliceExpr *slice) {
  ...
  char *start_expr = slice->start != NULL ? emit_expr(compiler, slice->start)
                                          : str_dup("sage_nil()");
  char *end_expr = slice->end != NULL ? emit_expr(compiler, slice->end)
                                      : str_dup("sage_nil()");
  if (compiler->failed) {
    free(array_expr);
    free(start_expr);
...
static char *emit_dict_expr(Compiler *compiler, DictExpr *dict) {
  ...
  sb_appendf(&sb, "sage_make_dict_from_entries(%d, (const char*[]){",
             dict->count);
...
static char *emit_call_expr(Compiler *compiler, CallExpr *call) {
  ...
  if (strcmp(callee_name, "str") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "str", "usage: str(value)",
                                   "1");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "io_readfile") == 0) {
    if (call->arg_count != 1)
      return str_dup("sage_nil()");
    char *arg = emit_expr(compiler, call->args[0]);
    sb_appendf(&sb, "sage_io_readfile(%s)", arg);
    free(arg);
    free(callee_name);
    return sb_take(&sb);
  }
  if (strcmp(callee_name, "io_writefile") == 0) {
    if (call->arg_count != 2)
      return str_dup("sage_nil()");
    char *a1 = emit_expr(compiler, call->args[0]);
    char *a2 = emit_expr(compiler, call->args[1]);
    sb_appendf(&sb, "sage_io_writefile(%s, %s)", a1, a2);
    free(a1);
    free(a2);
    free(callee_name);
    return sb_take(&sb);
  ...
  if (strcmp(callee_name, "io_writebytes") == 0) {
    ...
    char *a1 = emit_expr(compiler, call->args[0]);
    char *a2 = emit_expr(compiler, call->args[1]);
    sb_appendf(&sb, "sage_io_writebytes(%s, %s)", a1, a2);
    free(a1);
    free(a2);
    free(callee_name);
    return sb_take(&sb);
  ...
  if (strcmp(callee_name, "io_readbytes") == 0) {
    if (call->arg_count != 1)
      return str_dup("sage_nil()");
    char *arg = emit_expr(compiler, call->args[0]);
    sb_appendf(&sb, "sage_io_readbytes(%s)", arg);
    free(arg);
    free(callee_name);
    return sb_take(&sb);
  }
  ...
  if (strcmp(callee_name, "string_substr") == 0) {
    ...
    char *a1 = emit_expr(compiler, call->args[0]);
    char *a2 = emit_expr(compiler, call->args[1]);
    char *a3 = emit_expr(compiler, call->args[2]);
    sb_appendf(&sb, "sage_string_substr(%s, %s, %s)", a1, a2, a3);
    free(a1);
    free(a2);
    free(a3);
    free(callee_name);
  ...
  if (strcmp(callee_name, "len") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "len", "usage: len(value)",
                                   "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "push") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "push",
                                   "usage: push(array, value)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "pop") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "pop", "usage: pop(array)",
                                   "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "array_extend") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "array_extend",
                                   "usage: array_extend(target, extra)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "array_reverse") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "array_reverse",
                                   "usage: array_reverse(array)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "array_contains") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "array_contains",
                                   "usage: array_contains(array, needle)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "array_index_of") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "array_index_of",
                                   "usage: array_index_of(array, needle)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
      char *a = emit_expr(compiler, call->args[0]);
      char *b = emit_expr(compiler, call->args[1]);
  ...
  if (strcmp(callee_name, "tonumber") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "tonumber",
  ...
  if (strcmp(callee_name, "dict_keys") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "dict_keys",
                                   "usage: dict_keys(dict_value)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "dict_values") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "dict_values",
                                   "usage: dict_values(dict_value)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "dict_has") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "dict_has",
                                   "usage: dict_has(dict_value, key)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "gc_collect") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "gc_collect",
                                   "usage: gc_collect()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "gc_stats") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "gc_stats",
                                   "usage: gc_stats()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "gc_collections") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "gc_collections",
                                   "usage: gc_collections()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "gc_enable") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "gc_enable",
                                   "usage: gc_enable()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "gc_disable") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "gc_disable",
                                   "usage: gc_disable()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "upper") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "upper",
                                   "usage: upper(text)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "lower") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "lower",
                                   "usage: lower(text)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "strip") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "strip",
                                   "usage: strip(text)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "split") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "split",
                                   "usage: split(text, separator)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "join") == 0) {
    if (call->arg_count != 2) {
      compiler_builtin_arity_error(compiler, call, "join",
                                   "usage: join(parts, separator)", "2");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "replace") == 0) {
    if (call->arg_count != 3) {
      compiler_builtin_arity_error(compiler, call, "replace",
                                   "usage: replace(text, old, new)", "3");
      sb_append(&sb, "sage_nil()");
    } else {
      char *str_arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "mem_alloc") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "mem_alloc",
                                   "usage: mem_alloc(size)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "mem_free") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "mem_free",
                                   "usage: mem_free(pointer)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "mem_size") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "mem_size",
                                   "usage: mem_size(pointer)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "struct_def") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "struct_def",
                                   "usage: struct_def(fields)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "struct_new") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "struct_new",
                                   "usage: struct_new(definition)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "struct_size") == 0) {
    if (call->arg_count != 1) {
      compiler_builtin_arity_error(compiler, call, "struct_size",
                                   "usage: struct_size(definition)", "1");
      sb_append(&sb, "sage_nil()");
    } else {
      char *arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "clock") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "clock", "usage: clock()",
                                   "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  if (strcmp(callee_name, "input") == 0) {
    if (call->arg_count == 0) {
      ...
      compiler_builtin_arity_error(compiler, call, "input",
                                   "usage: input() or input(prompt)", "0 or 1");
      sb_append(&sb, "sage_nil()");
    }
    free(callee_name);
    return sb_take(&sb);
  ...
  if (strcmp(callee_name, "slice") == 0) {
    if (call->arg_count != 3) {
      compiler_builtin_arity_error(compiler, call, "slice",
                                   "usage: slice(value, start, end)", "3");
      sb_append(&sb, "sage_nil()");
    } else {
      char *array_arg = emit_expr(compiler, call->args[0]);
  ...
  if (strcmp(callee_name, "asm_arch") == 0) {
    if (call->arg_count != 0) {
      compiler_builtin_arity_error(compiler, call, "asm_arch",
                                   "usage: asm_arch()", "0");
      sb_append(&sb, "sage_nil()");
    } else {
  ...
  // Additional builtins: chr, ord, type, gc_disable, gc_enable, gc_collect,
  // gc_stats
...
static char *emit_set_expr(Compiler *compiler, SetExpr *set) {
  if (set->object != NULL) {
    ...
    sb_init(&sb);
    sb_appendf(&sb,
               "({SageValue _obj = %s; SageValue _val = %s; "
...
static char *emit_expr(Compiler *compiler, Expr *expr) {
  ...
  case EXPR_VARIABLE: {
    ...
    if (slot_name == NULL) {
      char help[256];
      compiler_error_at(
          compiler, &expr->as.variable.name,
          compiler_unknown_name_help(compiler, name,
                                     "declare the value first or spell the "
                                     "name exactly as it was defined",
                                     help, sizeof(help)),
...
static void emit_stmt(Compiler *compiler, Stmt *stmt) {
  ...
  case STMT_LET: {
    ...
    if (slot_name == NULL) {
      compiler_error_at(compiler, &stmt->as.let.name, NULL,
                        "internal compiler error: let target '%s' was not "
                        "collected for code generation",
                        name);
      free(name);
      break;
    }
  ...
  case STMT_FOR: {
    ...
    if (slot_name == NULL) {
      compiler_error_at(
          compiler, &stmt->as.for_stmt.variable, NULL,
    ...
    emit_line(compiler, "for (int %s = 0; %s < _len; %s++) {", idx_var, idx_var,
              idx_var);
    compiler->indent++;
    emit_line(compiler, "char _ch[2] = {%s.as.string[%s], '\\0'};", iter_var,
              idx_var);
    emit_line(compiler, "sage_define_slot(&%s, sage_string(_ch));", slot_name);
    emit_embedded_block(compiler, stmt->as.for_stmt.body);
    compiler->indent--;
    emit_line(compiler, "}");
    compiler->indent--;
  ...
  case STMT_TRY: {
    ...
    if (try_stmt->catch_count > 0) {
      ...
      if (catch_slot != NULL) {
        emit_line(compiler, "sage_define_slot(&%s, sage_exception_value);",
                  catch_slot);
      }
      free(catch_var);
    }
...
static void emit_runtime_prelude(FILE *out, CompilerTarget target) {
  ...
  fputs(
      ...
      "}\n"
      "\n"
      "static void sage_gc_recompute_thresholds(unsigned long reclaimed_bytes, "
      ...
      "    if (byte_padding < (SAGE_GC_MIN_TRIGGER_BYTES / 2)) byte_padding = "
      "SAGE_GC_MIN_TRIGGER_BYTES / 2;\n"
      "    if (object_padding < (SAGE_GC_MIN_TRIGGER_OBJECTS / 2)) "
      "object_padding = SAGE_GC_MIN_TRIGGER_OBJECTS / 2;\n"
      "    if (reclaimed_bytes <= live_bytes / 8) {\n"
      "        byte_padding /= 2;\n"
      "        if (byte_padding < (SAGE_GC_MIN_TRIGGER_BYTES / 2)) "
      "byte_padding = SAGE_GC_MIN_TRIGGER_BYTES / 2;\n"
      ...
      "    if (sage_gc.next_gc_bytes < SAGE_GC_MIN_TRIGGER_BYTES) "
      "sage_gc.next_gc_bytes = SAGE_GC_MIN_TRIGGER_BYTES;\n"
      "    sage_gc.next_gc_objects = live_objects + object_padding;\n"
      "    if (sage_gc.next_gc_objects < SAGE_GC_MIN_TRIGGER_OBJECTS) "
      "sage_gc.next_gc_objects = SAGE_GC_MIN_TRIGGER_OBJECTS;\n"
      "}\n"
      "\n"
      ...
      "\n",
      out);
  fputs("static size_t sage_gc_release_object(SageGcHeader* header) {\n"
        ...
        "\n",
        out);
  fputs("static void sage_gc_collect(void) {\n"
        ...
        "    }\n"
        "    sage_gc.collections++;\n"
        "    sage_gc_recompute_thresholds(before_bytes - sage_gc_live_bytes(), "
        "before_objects - sage_gc.object_count);\n"
        "}\n"
        "\n",
        out);
  fputs(
      ...
      "    if ((sage_gc.object_count + 1) >= sage_gc.next_gc_objects) return "
      "1;\n"
      "    return sage_gc_live_bytes() + (unsigned long)sizeof(SageGcHeader) + "
      "(unsigned long)incoming_size >= sage_gc.next_gc_bytes;\n"
      "}\n"
      "\n"
      "static void* sage_gc_alloc(SageGcKind kind, size_t size) {\n"
      "    if (sage_gc.frames != NULL && sage_gc_should_collect(size)) "
      "sage_gc_collect();\n"
      ...
      "}\n"
      "\n"
      "static void sage_gc_push_frame(SageGcFrame* frame, SageSlot** slots, "
      "int slot_count) {\n"
  ...
  fputs("static void sage_gc_mark_value(SageValue value) {\n"
        ...
        "sage_gc_mark_value(value.as.array->elements[i]);\n"
        "            }\n"
        "            return;\n"
        "        case SAGE_TAG_DICT:\n"
        ...
        "sage_gc_mark_value(value.as.tuple->elements[i]);\n"
        "            }\n"
        "            return;\n"
        "        default:\n"
        "            return;\n"
        ...
        "    SageArray* array = (SageArray*)sage_gc_alloc(SAGE_GC_ARRAY, "
        "sizeof(SageArray));\n"
        "    array->count = 0;\n"
        "    array->capacity = 0;\n"
        "    array->elements = NULL;\n"
        ...
        "}\n"
        "\n"
        "static SageValue sage_nil(void) { SageValue v; v.type = SAGE_TAG_NIL; "
        "v.as.number = 0; return v; }\n"
        "static SageValue sage_number(double value) { SageValue v; v.type = "
        "SAGE_TAG_NUMBER; v.as.number = value; return v; }\n"
        "static SageValue sage_bool(int value) { SageValue v; v.type = "
        "SAGE_TAG_BOOL; v.as.boolean = value ? 1 : 0; return v; }\n"
        "static SageValue sage_string(const char* value) { SageValue v; v.type "
        "= SAGE_TAG_STRING; v.as.string = sage_gc_copy_string(value == NULL ? "
        "\"\" : value); return v; }\n"
  ...
  fputs("static SageValue sage_make_dict(void) {\n"
        "    SageDict* dict = (SageDict*)sage_gc_alloc(SAGE_GC_DICT, "
        "sizeof(SageDict));\n"
        "    dict->keys = NULL;\n"
        "    dict->values = NULL;\n"
        "    dict->count = 0;\n"
        ...
        "}\n"
        "\n"
        "static void sage_dict_set(SageDict* dict, const char* key, SageValue "
        "value) {\n"
        "    for (int i = 0; i < dict->count; i++) {\n"
        ...
        "        dict->keys = (char**)realloc(dict->keys, sizeof(char*) * "
        "(size_t)cap);\n"
        "        dict->values = (SageValue*)realloc(dict->values, "
        "sizeof(SageValue) * (size_t)cap);\n"
        ...
        out);
  fputs(
      "static SageValue sage_make_dict_from_entries(int count, const char** "
      "keys, const SageValue* values) {\n"
      "    sage_gc_pin();\n"
      ...
      "    sage_gc_pin();\n"
      "    SageTuple* tuple = (SageTuple*)sage_gc_alloc(SAGE_GC_TUPLE, "
      "sizeof(SageTuple));\n"
      "    tuple->count = count;\n"
      "    tuple->elements = (SageValue*)malloc(sizeof(SageValue) * "
      "(size_t)count);\n"
      "    if (tuple->elements == NULL && count > 0) sage_fail(\"Runtime "
      "Error: out of memory\");\n"
      ...
      "    SageValue* elements = (SageValue*)realloc(array->elements, "
      "sizeof(SageValue) * (size_t)capacity);\n"
  ...
  fputs(
      ...
      "    if (value.type == SAGE_TAG_STRING) return value.as.string[0] != "
      "'\\0';\n"
      "    return 1;\n"
      "}\n"
      "\n"
      ...
      "}\n"
      "\n"
      "static SageValue sage_assign_slot(SageSlot* slot, const char* name, "
      "SageValue value) {\n"
      "    if (!slot->defined) {\n"
      "        fprintf(stderr, \"Runtime Error: Undefined variable '%s'.\\n\", "
      "name);\n"
      ...
      "        case SAGE_TAG_NUMBER: return left.as.number == "
      "right.as.number;\n"
      "        case SAGE_TAG_BOOL: return left.as.boolean == "
      "right.as.boolean;\n"
      "        case SAGE_TAG_STRING: return strcmp(left.as.string, "
      "right.as.string) == 0;\n"
      "        case SAGE_TAG_ARRAY: {\n"
      "            if (left.as.array == right.as.array) return 1;\n"
      "            if (left.as.array->count != right.as.array->count) return "
      "0;\n"
      "            for (int i = 0; i < left.as.array->count; i++) {\n"
      "                if (!sage_values_equal(left.as.array->elements[i], "
      "right.as.array->elements[i])) return 0;\n"
      "            }\n"
      "            return 1;\n"
      "        }\n"
      ...
      "                if (!sage_values_equal(left.as.tuple->elements[i], "
      "right.as.tuple->elements[i])) return 0;\n"
      "            }\n"
      "            return 1;\n"
      "        }\n"
      "    }\n"
  ...
  fputs(
      ...
      "    if (value.type == SAGE_TAG_STRING) return "
      "sage_number((double)strlen(value.as.string));\n"
      "    if (value.type == SAGE_TAG_ARRAY) return "
      "sage_number((double)value.as.array->count);\n"
      "    if (value.type == SAGE_TAG_DICT) return "
      ...
      "    if (collection.type == SAGE_TAG_ARRAY && index.type == "
      "SAGE_TAG_NUMBER) {\n"
      ...
      "    }\n"
      "    if (collection.type == SAGE_TAG_TUPLE && index.type == "
      "SAGE_TAG_NUMBER) {\n"
      "        int idx = (int)index.as.number;\n"
      "        if (idx < 0 || idx >= collection.as.tuple->count) return "
      "sage_nil();\n"
      "        return collection.as.tuple->elements[idx];\n"
      "    }\n"
      "    if (collection.type == SAGE_TAG_STRING && index.type == "
      "SAGE_TAG_NUMBER) {\n"
      "        int idx = (int)index.as.number;\n"
      ...
      "}\n"
      "\n"
      "static SageValue sage_slice(SageValue array, SageValue start, SageValue "
      "end) {\n"
      "    if (array.type != SAGE_TAG_ARRAY) return sage_nil();\n"
      "    sage_gc_pin();\n"
      "    int start_index = 0;\n"
      "    int end_index = array.as.array->count;\n"
      "    if (start.type == SAGE_TAG_NUMBER) start_index = "
      "(int)start.as.number;\n"
      "    else if (start.type != SAGE_TAG_NIL) { sage_gc_unpin(); return "
      "sage_nil(); }\n"
      "    if (end.type == SAGE_TAG_NUMBER) end_index = (int)end.as.number;\n"
      "    else if (end.type != SAGE_TAG_NIL) { sage_gc_unpin(); return "
      "sage_nil(); }\n"
      "    if (start_index < 0) start_index = array.as.array->count + "
      "start_index;\n"
      "    if (end_index < 0) end_index = array.as.array->count + end_index;\n"
      "    if (start_index < 0) start_index = 0;\n"
      "    if (end_index > array.as.array->count) end_index = "
      "array.as.array->count;\n"
      "    if (start_index >= end_index) { SageValue empty = sage_array(); "
      "sage_gc_unpin(); return empty; }\n"
      "    SageValue result = sage_array();\n"
      "    for (int i = start_index; i < end_index; i++) {\n"
      "        sage_array_push_raw(result.as.array, "
      "array.as.array->elements[i]);\n"
      "    }\n"
      "    sage_gc_unpin();\n"
      "    return result;\n"
      "}\n"
      ...
      "    if (array.type != SAGE_TAG_ARRAY || array.as.array->count == 0) "
      "return sage_nil();\n"
  ...
  fputs("static SageValue sage_array_extend(SageValue target, SageValue source) "
      ...
      "        memcpy(dst->elements + dst->count, src->elements, "
      "sizeof(SageValue) * (size_t)src->count);\n"
      "        dst->count += src->count;\n"
      "    }\n"
      "    return sage_nil();\n"
      "}\n"
  ...
  fputs(
      ...
      "    if (start.type != SAGE_TAG_NUMBER || end.type != SAGE_TAG_NUMBER) "
      "return sage_nil();\n"
      "    sage_gc_pin();\n"
      "    SageValue result = sage_array();\n"
  ...
  fputs(
      ...
      "sage_bool(sage_values_equal(left, right)); }\n"
      "static SageValue sage_neq(SageValue left, SageValue right) { return "
      "sage_bool(!sage_values_equal(left, right)); }\n"
      ...
      "    return sage_bool(left.as.number <= right.as.number);\n"
      "}\n"
      "static SageValue sage_not(SageValue value) { return "
      "sage_bool(!sage_truthy(value)); }\n"
      "static SageValue sage_and(SageValue left, SageValue right) { return "
      "sage_bool(sage_truthy(left) && sage_truthy(right)); }\n"
      "static SageValue sage_or(SageValue left, SageValue right) { return "
      "sage_bool(sage_truthy(left) || sage_truthy(right)); }\n"
      "static SageValue sage_bit_not(SageValue value) {\n"
      "    if (value.type != SAGE_TAG_NUMBER) sage_fail(\"Runtime Error: "
      "Bitwise NOT operand must be a number.\");\n"
  ...
  fputs(
      ...
      "    if (left.type != SAGE_TAG_NUMBER || right.type != SAGE_TAG_NUMBER) "
      "sage_fail(\"Runtime Error: Operands must be numbers.\");\n"
      "    return sage_number((double)(((long long)left.as.number) & ((long "
      "long)right.as.number)));\n"
      "}\n"
      ...
      "        if (end != value.as.string && *end == '\\0') return "
      "sage_number(result);\n"
      "    }\n"
      "    return sage_nil();\n"
      "}\n"
      ...
      "        sage_array_push_raw(result.as.array, "
      "sage_string(dict_val.as.dict->keys[i]));\n"
      "    }\n"
      "    sage_gc_unpin();\n"
      "    return result;\n"
      "}\n"
      ...
      "    if (dict_val.type != SAGE_TAG_DICT || key.type != SAGE_TAG_STRING) "
      "return sage_bool(0);\n"
      "    for (int i = 0; i < dict_val.as.dict->count; i++) {\n"
      "        if (strcmp(dict_val.as.dict->keys[i], key.as.string) == 0) "
      "return sage_bool(1);\n"
      "    }\n"
      "    return sage_bool(0);\n"
      "}\n"
  ...
  fputs(
      ...
      "    if (v.type != SAGE_TAG_STRING || v.as.string == NULL || "
      "v.as.string[0] == 0) return sage_nil();\n"
      ...
      "    if (s.type != SAGE_TAG_STRING || prefix.type != SAGE_TAG_STRING) "
      "return sage_bool(0);\n"
      "    return sage_bool(strncmp(s.as.string, prefix.as.string, "
      "strlen(prefix.as.string)) == 0);\n"
      "}\n"
      "\n"
      "static SageValue sage_endswith(SageValue s, SageValue suffix) {\n"
      "    if (s.type != SAGE_TAG_STRING || suffix.type != SAGE_TAG_STRING) "
      "return sage_bool(0);\n"
      "    size_t slen = strlen(s.as.string), suflen = "
      "strlen(suffix.as.string);\n"
      "    if (suflen > slen) return sage_bool(0);\n"
      "    return sage_bool(strcmp(s.as.string + slen - suflen, "
      "suffix.as.string) == 0);\n"
      "}\n"
      "\n"
      "static SageValue sage_contains(SageValue haystack, SageValue needle) {\n"
      "    if (haystack.type != SAGE_TAG_STRING || needle.type != "
      "SAGE_TAG_STRING) return sage_bool(0);\n"
      "    return sage_bool(strstr(haystack.as.string, needle.as.string) != "
      "NULL);\n"
      "}\n"
      "\n"
  ...
  fputs("static void sage_index_set(SageValue c, SageValue k, SageValue v) {\n"
        ...
        "            if (strcmp(d->keys[i], k.as.string) == 0) { d->values[i] "
        "= v; return; }\n"
        "        }\n"
        ...
        "            d->capacity = nc;\n"
        "        }\n"
        "        { size_t l = strlen(k.as.string); d->keys[d->count] = "
        "malloc(l+1); memcpy(d->keys[d->count], k.as.string, l+1); }\n"
        "        d->values[d->count] = v;\n"
        "        d->count++;\n"
        "    }\n"
        "}\n"
  ...
  fputs("static SageValue sage_gc_collect_fn(void) {\n"
        "    sage_gc_collect();\n"
        "    return sage_nil();\n"
        "}\n"
        "\n"
        ...
        "        (const char*[]){\"bytes_allocated\", \"current_bytes\", "
        "\"num_objects\", \"collections\", \"objects_freed\", \"next_gc\", "
        "\"next_gc_bytes\"},\n"
        "        (SageValue[]){\n"
  ...
  fputs("static SageValue sage_gc_collections_fn(void) {\n"
  ...
  fputs("#include <ctype.h>\n"
        ...
        "(char)toupper((unsigned char)value.as.string[i]);\n"
        "    result[len] = '\\0';\n"
        "    return sage_string_take(result);\n"
        "}\n"
        ...
        "    if (result == NULL) sage_fail(\"Runtime Error: out of memory\");\n"
        "    for (size_t i = 0; i < len; i++) result[i] = "
        "(char)tolower((unsigned char)value.as.string[i]);\n"
        "    result[len] = '\\0';\n"
        "    return sage_string_take(result);\n"
        "}\n"
  ...
  fputs(
      ...
      "{\n"
      "    if (str_val.type != SAGE_TAG_STRING || delim_val.type != "
      "SAGE_TAG_STRING) return sage_array();\n"
      "    sage_gc_pin();\n"
      ...
      "{\n"
      "    if (arr_val.type != SAGE_TAG_ARRAY || delim_val.type != "
      "SAGE_TAG_STRING) return sage_nil();\n"
      ...
      "        if (arr->elements[i].type == SAGE_TAG_STRING) total += "
      "strlen(arr->elements[i].as.string);\n"
      "        if (i > 0) total += dlen;\n"
      "    }\n"
  ...
  fputs("static SageValue sage_replace_fn(SageValue str_val, SageValue "
        "old_val, SageValue new_val) {\n"
        "    if (str_val.type != SAGE_TAG_STRING || old_val.type != "
        "SAGE_TAG_STRING || new_val.type != SAGE_TAG_STRING)\n"
        "        return sage_nil();\n"
        "    const char* s = str_val.as.string;\n"
        ...
        "    size_t count = 0;\n"
        "    const char* tmp = s;\n"
        "    while ((tmp = strstr(tmp, old_s)) != NULL) { count++; tmp += "
        "old_len; }\n"
  ...
  fputs(
      "static SageValue sage_mem_read(SageValue ptr_val, SageValue off_val, "
      "SageValue type_val) {\n"
      "    SagePointer* sp = sage_as_pointer(ptr_val);\n"
      ...
      "    if (strcmp(type, \"double\") == 0) { double v; memcpy(&v, base, "
      "sizeof(double)); return sage_number(v); }\n"
      "    if (strcmp(type, \"string\") == 0) { return sage_string((const "
      "char*)base); }\n"
      "    return sage_nil();\n"
      "}\n"
      "\n"
      "static SageValue sage_mem_write(SageValue ptr_val, SageValue off_val, "
      "SageValue type_val, SageValue val) {\n"
      "    SagePointer* sp = sage_as_pointer(ptr_val);\n"
      "    if (sp == NULL || sp->ptr == NULL || off_val.type != "
      "SAGE_TAG_NUMBER || type_val.type != SAGE_TAG_STRING)\n"
      "        return sage_nil();\n"
      ...
      "    else if (strcmp(type, \"double\") == 0 && val.type == "
      "SAGE_TAG_NUMBER) { double v = val.as.number; memcpy(base, &v, "
      "sizeof(double)); }\n"
      "    return sage_nil();\n"
      "}\n"
      "\n"
  ...
  /* Struct builtins */
  fputs("static int sage_struct_type_info(const char* type, size_t* out_size, "
        "size_t* out_align) {\n"
        "    if (strcmp(type,\"char\")==0||strcmp(type,\"byte\")==0) { "
        "*out_size=1; *out_align=1; return 0; }\n"
        "    if (strcmp(type,\"short\")==0) { *out_size=sizeof(short); "
        "*out_align=sizeof(short); return 0; }\n"
        "    if (strcmp(type,\"int\")==0) { *out_size=sizeof(int); "
        "*out_align=sizeof(int); return 0; }\n"
        "    if (strcmp(type,\"long\")==0) { *out_size=sizeof(long); "
        "*out_align=sizeof(long); return 0; }\n"
        "    if (strcmp(type,\"float\")==0) { *out_size=sizeof(float); "
        "*out_align=sizeof(float); return 0; }\n"
        "    if (strcmp(type,\"double\")==0) { *out_size=sizeof(double); "
        "*out_align=sizeof(double); return 0; }\n"
        "    if (strcmp(type,\"ptr\")==0) { *out_size=sizeof(void*); "
        "*out_align=sizeof(void*); return 0; }\n"
        "    return -1;\n"
        "}\n"
        "\n"
  ...
  fputs("        /* store field: \"name\" -> [offset, type] */\n"
        ...
        "    sage_dict_set(def.as.dict, \"__size__\", "
        "sage_number((double)offset));\n"
        "    sage_dict_set(def.as.dict, \"__align__\", "
        "sage_number((double)max_align));\n"
        "    sage_gc_unpin();\n"
        "    return def;\n"
        "}\n"
        "\n"
  ...
  fputs(
      "static SageValue sage_struct_get(SageValue ptr_val, SageValue def, "
      "SageValue field_name) {\n"
      "    SagePointer* sp = sage_as_pointer(ptr_val);\n"
      "    if (sp == NULL || sp->ptr == NULL || def.type != SAGE_TAG_DICT || "
      "field_name.type != SAGE_TAG_STRING)\n"
      "        return sage_nil();\n"
      "    SageValue info = sage_dict_get(def.as.dict, field_name.as.string);\n"
      "    if (info.type != SAGE_TAG_ARRAY || info.as.array->count < 2) return "
      "sage_nil();\n"
      "    size_t offset = (size_t)info.as.array->elements[0].as.number;\n"
      "    const char* type = info.as.array->elements[1].as.string;\n"
      "    unsigned char* base = (unsigned char*)sp->ptr + offset;\n"
      "    if (strcmp(type,\"char\")==0||strcmp(type,\"byte\")==0) return "
      "sage_number((double)*base);\n"
      "    if (strcmp(type,\"short\")==0) { short v; "
      "memcpy(&v,base,sizeof(short)); return sage_number((double)v); }\n"
      "    if (strcmp(type,\"int\")==0) { int v; memcpy(&v,base,sizeof(int)); "
      "return sage_number((double)v); }\n"
      "    if (strcmp(type,\"long\")==0) { long v; "
      "memcpy(&v,base,sizeof(long)); return sage_number((double)v); }\n"
      "    if (strcmp(type,\"float\")==0) { float v; "
      "memcpy(&v,base,sizeof(float)); return sage_number((double)v); }\n"
      "    if (strcmp(type,\"double\")==0) { double v; "
      "memcpy(&v,base,sizeof(double)); return sage_number(v); }\n"
      "    return sage_nil();\n"
      "}\n"
      "\n"
      "static SageValue sage_struct_set(SageValue ptr_val, SageValue def, "
      "SageValue field_name, SageValue val) {\n"
      "    SagePointer* sp = sage_as_pointer(ptr_val);\n"
      "    if (sp == NULL || sp->ptr == NULL || def.type != SAGE_TAG_DICT || "
      "field_name.type != SAGE_TAG_STRING)\n"
      "        return sage_nil();\n"
      ...
      "    if (strcmp(type,\"char\")==0||strcmp(type,\"byte\")==0) { *base = "
      "(unsigned char)val.as.number; }\n"
      "    else if (strcmp(type,\"short\")==0) { short v=(short)val.as.number; "
      "memcpy(base,&v,sizeof(short)); }\n"
      "    else if (strcmp(type,\"int\")==0) { int v=(int)val.as.number; "
      "memcpy(base,&v,sizeof(int)); }\n"
      "    else if (strcmp(type,\"long\")==0) { long v=(long)val.as.number; "
      "memcpy(base,&v,sizeof(long)); }\n"
      "    else if (strcmp(type,\"float\")==0) { float v=(float)val.as.number; "
      "memcpy(base,&v,sizeof(float)); }\n"
      "    else if (strcmp(type,\"double\")==0) { double v=val.as.number; "
  ...
  fputs("typedef SageValue (*SageMethodFn)(SageValue, int, SageValue*);\n"
        ...
        "{\n"
        "    if (sage_class_count >= SAGE_MAX_CLASSES) sage_fail(\"too many "
        "classes\");\n"
        ...
        "SageMethodFn fn) {\n"
        "    if (sage_method_count >= SAGE_MAX_METHODS) sage_fail(\"too many "
        "methods\");\n"
  ...
  fputs("static SageValue sage_arch_fn(void) {\n"
  ...
  if (target != COMPILER_TARGET_PICO) {
    fputs(
        ...
        "    if (prompt.type == SAGE_TAG_STRING) fputs(prompt.as.string, "
        "stdout);\n"
        "    char buf[4096];\n"
        ...
        "    for(int i=0; i<sage_argc; i++) sage_push(list, "
        "sage_string(sage_argv[i]));\n"
        "    return list;\n"
        "}\n"
        ...
        "    if(p.type != SAGE_TAG_STRING || c.type != SAGE_TAG_STRING) return "
        "sage_bool(0);\n"
        "    FILE* f = fopen(p.as.string, \"wb\"); if(!f) return "
        "sage_bool(0);\n"
        "    fwrite(c.as.string, 1, strlen(c.as.string), f); fclose(f); return "
        "sage_bool(1);\n"
        "}\n"
        "static SageValue sage_io_writebytes(SageValue p, SageValue arr) {\n"
        "    if(p.type != SAGE_TAG_STRING || arr.type != SAGE_TAG_ARRAY) "
        "return sage_bool(0);\n"
        "    FILE* f = fopen(p.as.string, \"wb\"); if(!f) return "
        "sage_bool(0);\n"
        "    SageArray* a = arr.as.array;\n"
        "    unsigned char* buf = malloc(a->count);\n"
        "    for(int i=0; i<a->count; i++) buf[i] = (unsigned "
        "char)a->elements[i].as.number;\n"
        "    fwrite(buf, 1, a->count, f); fclose(f); free(buf); return "
        "sage_bool(1);\n"
        "}\n"
...
static void emit_method_definition(Compiler *compiler, ClassInfo *cls,
  ...
  int has_self = (proc->param_count > 0 && proc->params[0].length == 4 &&
  ...
  fprintf(compiler->out,
          "static SageValue sage_method_%s_%s(SageValue _self, int _argc, "
          "SageValue* _argv) {\n",
  ...
  for (ClassInfo *cls = compiler->classes; cls != NULL; cls = cls->next) {
    for (Stmt *method = cls->methods; method != NULL; method = method->next) {
      if (method->type == STMT_PROC) {
        ...
        emit_indent(compiler);
        fprintf(compiler->out,
                "static SageValue sage_method_%s_%s(SageValue _self, int "
                "_argc, SageValue* _argv);\n",
                cls->class_name, method_name);
        free(method_name);
      }
    }
...
static void emit_main_function(Compiler *compiler, Stmt *program,
  ...
  for (ClassInfo *cls = compiler->classes; cls != NULL; cls = cls->next) {
    if (cls->parent_name) {
      ...
      emit_line(compiler, "sage_register_class(\"%s\", NULL);",
                cls->class_name);
    }
    for (Stmt *method = cls->methods; method != NULL; method = method->next) {
      if (method->type == STMT_PROC) {
        ...
        emit_line(compiler,
                  "sage_register_method(\"%s\", \"%s\", sage_method_%s_%s);",
...
static int write_pico_cmake_lists(const char *cmake_path,
...
static int write_c_output_internal(const char *source, const char *input_path,
                                   const char *output_path,
                                   CompilerTarget target, int opt_level,
                                   int debug_info) {
...
int compile_source_to_c(const char *source, const char *input_path,
                        const char *output_path) {
  return write_c_output_internal(source, input_path, output_path,
                                 COMPILER_TARGET_HOST, 0, 0);
}

int compile_source_to_c_opt(const char *source, const char *input_path,
                            const char *output_path, int opt_level,
                            int debug_info) {
  return write_c_output_internal(source, input_path, output_path,
                                 COMPILER_TARGET_HOST, opt_level, debug_info);
}
...
int compile_source_to_executable(const char *source, const char *input_path,
  ...
  if (!write_c_output_internal(source, input_path, c_output_path,
                               COMPILER_TARGET_HOST, 0, 0)) {
    return 0;
  }

  const char *cc =
      (cc_command != NULL && cc_command[0] != '\0') ? cc_command : "cc";
  pid_t pid = fork();
  if (pid < 0) {
...
int compile_source_to_executable_opt(const char *source, const char *input_path,
                                     const char *c_output_path,
                                     const char *exe_output_path,
                                     const char *cc_command, int opt_level,
                                     int debug_info) {
  if (!write_c_output_internal(source, input_path, c_output_path,
                               COMPILER_TARGET_HOST, opt_level, debug_info)) {
    return 0;
...
int compile_source_to_pico_uf2(const char *source, const char *input_path,
  ...
  if (sdk_path == NULL || sdk_path[0] == '\0') {
    fprintf(stderr, "Compiler error: PICO_SDK_PATH is not set. Use --sdk or "
  ...
  if (realpath(effective_output_dir, resolved_output_dir) == NULL) {
    fprintf(stderr,
            "Compiler error: could not resolve output directory \"%s\": %s\n",
  ...
  if (repo_root == NULL) {
    fprintf(stderr, "Compiler error: could not locate repo root containing "
  ...
  if (!path_exists(uf2_path)) {
    fprintf(stderr,
            "Compiler error: expected UF2 was not produced at \"%s\".\n",
            uf2_path);
    free(uf2_path);
    free(repo_root);
    free(import_path);
